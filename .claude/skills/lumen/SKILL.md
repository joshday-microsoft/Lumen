...
- **Art vanishes hours after a successful performance, and the log says `canvas pushed`:** the BLE link dropped and the daemon "restored" the screen through the image-upload path, which is the unreliable one on this unit — every write acks, nothing renders, and the wall sits dark until a human notices (2026-08-05: a 3-hour drop killed that morning's painting). **Fixed in the daemon 2026-08-05**: `restore_display()` now re-sends a GIF through the GIF flow, and repaints a still pixel-by-pixel through Graffiti (~1024 writes, ~13 s), after the image upload rather than instead of it — the upload is still what the panel stores for its own power-up, the repaint is what guarantees it is visible. It skips self-driving modes (games, life, clock, text, off) and never fights an in-flight painting, since `paint_runner` resumes on its own pixel. A daemon restart also reloads `tmp/canvas.png` as the boot canvas, so restarting no longer throws away the day's art.
