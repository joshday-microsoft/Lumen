...
- Live pixel painting (no GIF at all): `POST /spiral {delay}` fills the panel one LED at a time through the spectrum; Graffiti mode (`Graffiti.setPixel`) draws single pixels in real time — good for slow generative art and status indicators. Completed spiral saved as `art\spectrum-spiral.png`.
